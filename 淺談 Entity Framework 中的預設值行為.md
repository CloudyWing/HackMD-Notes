...
### Entity Framework 測試
使用 .NET 8，安裝 Microsoft.EntityFrameworkCore 8，並使用反向工程建立 Entity Framework。產生的程式如下：
```csharp
...
OUTPUT INSERTED.[Id], INSERTED.[TestInt]
VALUES (@p0);
...
執行結果如下：
| Name | TestInt | TestBit |
| --- | --- | --- |
| Name | 1234 | 0 |
| Name | 1234 | 0 |

看了以上結果稍微鬆了一口氣，`TestInt` 被忽略了，但 `TestBit` 並沒有，這代表新增時，是否會忽略欄位，還是會根據型別來判定。不然如果遇到預設值 為`true` 的 SQL 欄位，要寫入 `false`，結果實際寫成 `true`，那這個畫面太美，我不敢看。
...
```csharp
INSERT INTO [Test2] ([TestBit], [TestInt])
OUTPUT INSERTED.[Id]
VALUES (@p0, @p1);
...
由以上結果可以得知，當未設定 SQL 預設值時，當 Entity 屬性或給值，或是給與 C# 初始值相同的值時，產生的 INSERT 語法，並不會忽略欄位。

## 結論
1. 當 SQL Server 欄位有設定預設值時，Entity Framework 在新增資料時，如果 Entity 屬性值與該型別的預設值一致，某些型別的欄位在產生 INSERT 語法時會被忽略。
2. 使用 Entity Framework 時，應確保 SQL 預設值與 C# 預設值一致，以避免意外結果。但對於字串型別，可將其設為 `NOT NULL Default ''`，以統一處理 null 和空字串。

:::info
雖然這裡沒有測試 `DateTime` 型別，但其結果應類似於 `int`。不同的是，C# 的 `DateTime` 型別預設值為 `0001/01/01 00:00:00`，而 SQL Server 的不同日期型別會有不同允許的日期區間。例如，`datetime` 型別的日期區間為 `1753-01-01` 至 `9999-12-31`，因此會無法寫入資料。
:::
...
```csharp
using (TestContext context = new(dbContextOptions)) {
    Test entity = context.Tests.Single(x => x.Id == 1);
