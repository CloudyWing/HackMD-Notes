...
:::info
憑證設定部分未經實際測試，可能有其他需要注意的配置細節（如證書格式、路徑權限等）。正式環境使用前建議參考 [Mosquitto TLS 官方文件](https://mosquitto.org/man/mosquitto-conf-5.html)。
:::
...
###### tags: `MQTT` `MQTTnet` `Mosquitto` `Docker` `.NET`