...
Assembly 一般會有以下幾種版本號，版本號由大於或等於 0 整數組合，最大值為 `UInt16.MaxValue - 1`。
    ...
    * Version：設定專案的 AssemblyInfoVersion，提供給套件辨識用的版本號，如果未設定 `PackageVersion`，NuGet 版本會使用此設定，格式「{Major}.{Minor}.{Patch}」，可在符合 [SemVer](https://semver.org/lang/zh-TW/) 的情況下，增加後綴詞。
    ...
    各變數含意：
    * Major：主版號，具有相同名稱但不同主要版本的元件不可互換。 較高的版本號碼可能表示產品的主要重寫，無法假設回溯相容性。
    * Minor：次版號，如果兩個元件上的名稱和主要版本號碼相同，但次要版本號碼不同，這表示具有回溯相容性的重大增強功能。 這個較高的次要版本號碼可能表示產品的點發行或產品的完全回溯相容新版本。
    * Build：組建編號，表示重新編譯相同的來源。當處理器、平臺或編譯器變更時，可能會使用不同的組建編號。
    * Revision：修訂號：具有相同名稱、主要和次要版本號碼但不同修訂的元件，目的是要完全互換。 在修正先前發行之元件中的安全性漏洞的組建中，可能會使用較高的修訂編號。
...
###### tags: `ASP.NET` `NuGet` `NuGet Package`
