---
title: "有關 .NET 技術文檔與命名規範的演變"
date: 2024-09-20
lastmod: 2024-09-20
description: "觀察 .NET 技術生態的變遷，記錄 MSDN 轉型為 Microsoft Learn 的過程，以及 C# 命名規範 (如 Private Field 前綴 `_`) 在官方文件與 StyleCop 規則中的演變與差異。"
tags: [".NET"]
---
...
過去，.NET 的技術文件主要集中在 MSDN（Microsoft Developer Network）上，這個平台為開發者提供了 API 參考、教程和範例程式碼。
...
在「[MSDN 程式碼庫已淘汰](https://learn.microsoft.com/zh-tw/teamblog/msdn-code-gallery-retired)」和「[MSDN 與 TechNet 移轉至 docs.microsoft.com 的更新](https://learn.microsoft.com/zh-tw/teamblog/msdn-technet-migration)」都說明了這一轉變。如果想搜尋舊版的 .NET 文件，可以在「[.NET 舊版文件](https://learn.microsoft.com/zh-tw/previous-versions/dotnet/)」中找到，不過由於文件更新策略分為 .NET Framework 4 之前的版本和持續更新的最新版本，部分舊資料可能已不再完整。
