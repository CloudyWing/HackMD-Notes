---
title: "淺談 C# 的 GetHashCode()"
date: 2024-09-01
lastmod: 2024-09-01
description: "探討 C# `GetHashCode()` 的實作原則及其在 Dictionary 與 HashSet 中的作用。強調覆寫 `Equals()` 時必須同時覆寫 `GetHashCode()` 以避免雜湊碰撞或物件比對錯誤。"
tags: ["C#"]
---
...
![gethashcode overwrite warning](images/%E6%B7%BA%E8%AB%87%20CSharp%20%E7%9A%84%20GetHashCode%28%29/gethashcode-overwrite-warning.png)
...
![vs generate equals menu](images/%E6%B7%BA%E8%AB%87%20CSharp%20%E7%9A%84%20GetHashCode%28%29/vs-generate-equals-menu.png)

選擇要作為 `Equals()` 判斷的成員，也可以選擇一同實作 `IEquatable<T>` 和 運算子（`＝=` 和 `！=`），這邊先不勾選這些額外專案：

![vs generate equals dialog](images/%E6%B7%BA%E8%AB%87%20CSharp%20%E7%9A%84%20GetHashCode%28%29/vs-generate-equals-dialog.png)
