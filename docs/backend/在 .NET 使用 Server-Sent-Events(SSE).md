---
title: "在 .NET 使用 Server-Sent-Events(SSE)"
date: 2023-03-15
lastmod: 2024-02-17
description: "介紹如何在 .NET 中實作 SSE (Server-Sent-Events) 即時通訊，比較了 SSE 與 Polling、WebSocket 的差異，並提供前端 JavaScript 的實作範例與事件監聽說明。"
...
* [Server-Sent-Events (SSE)](https://developer.mozilla.org/zh-TW/docs/Web/API/Server-sent_events/Using_server-sent_events)：一種單向連線，由 Server 向 Client 推送更新資料。此種方式有以下優點：單向連線，可降低伺服器負擔，只有一個 TCP 連線，減少頻寬浪費。
