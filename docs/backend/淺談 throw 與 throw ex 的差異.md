---
title: "淺談 throw 與 throw ex 的差異"
date: 2025-07-31
lastmod: 2025-07-31
description: "解析 C# 中 `throw` 與 `throw ex` 的關鍵差異。強調 `throw` 能保留原始 Stack Trace，有助於除錯；而 `throw ex` 會重置堆疊資訊，應避免使用。"
tags: [".NET","C#"]
---
...
也許是 `throw ex` 太多人誤用了，從 .NET 5 開始，增加的 `CA2200` 的檢核規則，當偵測到 `throw ex` 的程式碼，會出現警告 `CA2200: 重新擲回攔截到的例外狀況變更堆疊資訊`，有關 `CA2200` 的說明請參考 [重大變更：CA2200：重新擲回以儲存堆疊詳細資料](https://learn.microsoft.com/zh-tw/dotnet/core/compatibility/code-analysis/5.0/ca2200-rethrow-to-preserve-stack-details) 和 [CA2200: 必須重新擲回以儲存堆疊詳細資料](https://learn.microsoft.com/zh-tw/dotnet/fundamentals/code-analysis/quality-rules/ca2200)。
