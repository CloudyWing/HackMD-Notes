---
title: "使用 Windows Docker Desktop 建立 Oracle 環境"
date: 2022-10-24
lastmod: 2022-10-24
description: "教學如何在 Windows Docker Desktop 環境下建立 Oracle 資料庫容器。由於 Oracle 官方 Docker Image 不再提供直接下載，需從 GitHub 下載 Dockerfile 並手動建置 (Build) Image。"
tags: ["Oracle","Docker"]
---
...
6. 一般不需要使用`-o`，`-i`等有「MD5 checksums」錯誤導致失敗，再加上就好。
7. 如果出現image不存在的錯誤，開啟「dockerfile\{[|.ee|.se2|.xe]\}」，搜尋關鍵字`FROM oraclelinux`查看使用 image tag，再至[Oracle Docker Hub](https://hub.docker.com/_/oraclelinux)查詢tag是否不存在，改成已存在的tag，現今應該使用「7-slim」。
8. 靜置image建立完成，需等一段時間。
9. 建立docker-compose檔案，參考如下(標示大括弧的地方請置換成自己的設定)：
...
10. 靜置container建立完成，需等很長一段時間，不確定哪時完成，可以開啟「Docker Dashboard」查看container的log。

## 建立使用者

1. 開啟資料庫管理工具，靘? 「Oracle SQL Developer」、「sqlplus」，輸入帳號：SYS，密碼為docker-compose的ORACLE_PWD所設定的值，角色：sysdba。
