---
title: "如何在 Docker 使用 GitLab CI"
date: 2022-10-24
lastmod: 2022-10-24
description: "詳細教學如何在 Docker 環境中安裝並設定 GitLab 與 GitLab Runner。文章包含 Docker Compose 配置範例、Runner 註冊流程，以及一個使用 .NET 6 專案進行 Build、Test、Deploy 的完整 CI/CD 實例。"
...
參考[官方文件](https://docs.gitlab.com/ee/install/docker.html)，撰寫Docker Compose。
...
安裝方法可參考[官方文件](https://docs.gitlab.com/ee/install/docker.html)來設定 docker-compose，這邊將 GitLab Runner 的內容加至原先的 docker-compose.yml 裡面。
...
GitLab 有三種 Scope 的 Runner 類型可以註冊，可參閱「[The scope of runners](https://docs.gitlab.com/ee/ci/runners/runners_scope.html)」，取得 Token 的位置如下表：
    ...
    | Scope                   | 描述                       | Token位置                                                                                        |
    | ----------------------- | -------------------------- | ------------------------------------------------------------------------------------------------ |
    | Shared Runner           | 每個專案都可以使用。       | GitLab Admin Area > Overview > Runners > 點擊按鈕「Register an instance runner」則會顯示 Token。 |
...
輸入一些給此 Runner 其他開發維護人員需了解的相關的資訊，可以輸入空白。
...
![gitlab artifact download interface](images/%E5%A6%82%E4%BD%95%E5%9C%A8%20Docker%20%E4%BD%BF%E7%94%A8%20GitLab%20CI/gitlab-artifact-download-ui.png)

#### Deployments > Environments

會顯示一個可用環境，點擊「開啟」後，會開啟網址為 `http://127.0.0.1:9080` 的網頁，不從這邊開啟，直接輸入網址也可以。

![gitlab environment interface](images/%E5%A6%82%E4%BD%95%E5%9C%A8%20Docker%20%E4%BD%BF%E7%94%A8%20GitLab%20CI/gitlab-environment-ui.png)
...
4. Image 名稱只能為小寫，因為範例裡的 Repository 名稱有含大寫，所以使用`CI_PROJECT_PATH_SLUG`。
:::
